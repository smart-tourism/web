...
export default function CustomerFeedbackPraises() {
  const searchParams = useSearchParams();
  const categoryParam = searchParams.get("category");
  const location = searchParams.get("location") || "";

  const [activeTab, setActiveTab] = useState<ReviewCategory>("bagus");
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `/api/dashboard/praises?tempat_wisata=${encodeURIComponent(location)}`
        );
    ...
    <ContentLayout title="Dashboard">
      ...
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Praises</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex h-full flex-col gap-8 text-black mt-6">
        <div className="text-2xl font-bold">Customer Feedback: Praises</div>
